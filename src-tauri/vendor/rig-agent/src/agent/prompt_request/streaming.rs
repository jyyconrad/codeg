...
use crate::{
    ...
    agent::hook::{
        AgentHook, CommittedMessages, CommittedMessagesAction, CommittedMessagesNext, HookContext,
        HookStack, InvalidToolCallAction, ModelSelection, ModelSelectionAction, ModelTurnFinished,
        ReasoningDelta, StepEventKind, StreamResponseFinish, TextDelta, ToolCallDelta,
    },
    ...
    S: TurnSource,
{
    async_stream::stream! {
        ...
        'outer: loop {
                    ...
                    let stop_reason = match runner
                        ...
                        .on_messages_committed(
                            ...
                            CommittedMessages {
                                ...
                                next: CommittedMessagesNext::Failed,
                            },
                        )
                        .await
                    {
                        CommittedMessagesAction::Stop(reason) => Some(reason),
                        CommittedMessagesAction::Continue => None,
                    };
                    store_error_usage(&runner, &run);
            ...
            let next = match &step {
                AgentRunStep::CallModel { .. } => CommittedMessagesNext::CallModel,
                AgentRunStep::CallTools { .. } => CommittedMessagesNext::CallTools,
                AgentRunStep::Done(_) => CommittedMessagesNext::Done,
            };
            match runner
                .config
                .hooks
                ...
                .await
            {
                CommittedMessagesAction::Stop(reason) => {
                    store_error_usage(&runner, &run);
                    yield Err(StreamingError::Prompt(Box::new(run.cancel_error(reason))));
                    break 'outer;
                }
                CommittedMessagesAction::Continue => {}
            }
