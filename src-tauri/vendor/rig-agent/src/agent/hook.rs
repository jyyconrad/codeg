...
/// What the driver will do after this committed-messages checkpoint.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CommittedMessagesNext {
    /// Next step is a model completion request.
    CallModel,
    /// Next step is tool execution.
    CallTools,
    /// The run has completed successfully.
    Done,
    /// `next_step` returned an error (budget, cancel, protocol).
    Failed,
}

/// Checkpoint of this run's newly committed messages.
///
/// Fired after [`crate::agent::AgentRun::next_step`] returns and before the
/// step is dispatched (or after a `next_step` error).
#[derive(Clone, Copy)]
pub struct CommittedMessages<'a> {
    /// `AgentRun::messages()` — this run's new messages (prompt + assistants + tool results), NOT input history.
    pub messages: &'a [Message],
    /// The driver's next action, or [`CommittedMessagesNext::Failed`].
    pub next: CommittedMessagesNext,
}

/// Action for [`AgentHook::on_messages_committed`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum CommittedMessagesAction {
    /// Continue the run.
    Continue,
    /// Stop the run with a reason.
    Stop(String),
...
pub trait AgentHook: WasmCompatSend + WasmCompatSync {
    ...
    /// Observes this run's committed messages after `next_step` and before the
    /// step is dispatched (or after a `next_step` error).
    ///
    /// The default action continues the run so existing hooks keep compiling.
    fn on_messages_committed(
        &self,
        _ctx: &HookContext,
        _event: CommittedMessages<'_>,
    ) -> impl Future<Output = CommittedMessagesAction> + WasmCompatSend {
        async { CommittedMessagesAction::Continue }
    }
...
impl ShortCircuitAction for CommittedMessagesAction {
    const CONTINUE: Self = CommittedMessagesAction::Continue;
    fn is_continue(&self) -> bool {
        matches!(self, CommittedMessagesAction::Continue)
    }
}
...
impl AgentHook for HookStack {
    ...
    stack_first_non_continue! {
        ...
        on_messages_committed, messages_committed, CommittedMessages, CommittedMessagesAction;
...
mod tests {
    ...
    struct CommittedStopThenCount {
        stop: bool,
        calls: Arc<AtomicUsize>,
    }

    impl AgentHook for CommittedStopThenCount {
        async fn on_messages_committed(
            ...
            self.calls.fetch_add(1, Ordering::Relaxed);
            if self.stop {
                CommittedMessagesAction::stop("terminal")
            } else {
    ...
    #[tokio::test]
    async fn committed_messages_stop_short_circuits_later_hooks() {
        let calls = Arc::new(AtomicUsize::new(0));
        let mut stack = HookStack::with(CommittedStopThenCount {
            stop: true,
            calls: calls.clone(),
        });
        stack.push(CommittedStopThenCount {
            stop: false,
            calls: calls.clone(),
        });
        ...
        let action = stack
            .on_messages_committed(
                ...
                CommittedMessages {
                    messages: &messages,
                    next: CommittedMessagesNext::CallTools,
                },
            )
            .await;

        assert_eq!(action, CommittedMessagesAction::stop("terminal"));
        assert_eq!(calls.load(Ordering::Relaxed), 1);
    }

    fn messages_contain_assistant_tool_call(messages: &[Message]) -> bool {
    ...
    #[derive(Clone)]
    struct RecordingCommittedHook {
        events: Arc<Mutex<Vec<(usize, CommittedMessagesNext)>>>,
        saw_tool_call: Arc<AtomicBool>,
        call_tools_had_tool_call: Arc<AtomicBool>,
        call_tools_had_assistant: Arc<AtomicBool>,
    }

    impl RecordingCommittedHook {
        fn new() -> Self {
            Self {
                events: Arc::new(Mutex::new(Vec::new())),
                saw_tool_call: Arc::new(AtomicBool::new(false)),
                call_tools_had_tool_call: Arc::new(AtomicBool::new(false)),
                call_tools_had_assistant: Arc::new(AtomicBool::new(false)),
            }
        }

        fn events(&self) -> Vec<(usize, CommittedMessagesNext)> {
            self.events
                .lock()
    ...
    impl AgentHook for RecordingCommittedHook {
        async fn on_messages_committed(
            &self,
            _ctx: &HookContext,
            event: CommittedMessages<'_>,
        ) -> CommittedMessagesAction {
            if event.next == CommittedMessagesNext::CallTools {
                self.call_tools_had_tool_call
                    .store(self.saw_tool_call.load(Ordering::SeqCst), Ordering::SeqCst);
                self.call_tools_had_assistant.store(
                    messages_contain_assistant_tool_call(event.messages),
                    Ordering::SeqCst,
                );
            }
            self.events
                .lock()
                .unwrap_or_else(|error| error.into_inner())
                .push((event.messages.len(), event.next));
            CommittedMessagesAction::continue_run()
        }

        async fn on_tool_call(&self, _ctx: &HookContext, _event: ToolCall<'_>) -> ToolCallAction {
            self.saw_tool_call.store(true, Ordering::SeqCst);
            ToolCallAction::run()
        }
    }

    #[tokio::test]
    async fn on_messages_committed_sees_assistant_before_tool_body_and_done() {
        let hook = RecordingCommittedHook::new();
        ...
        let output = agent
            ...
            .max_turns(2)
            .await
            .expect("tool-then-text run should succeed");
        assert_eq!(output, "the answer is 5");

        let events = hook.events();
        assert_eq!(
            events.iter().map(|(_, next)| *next).collect::<Vec<_>>(),
            vec![
                CommittedMessagesNext::CallModel,
                CommittedMessagesNext::CallTools,
                CommittedMessagesNext::CallModel,
                CommittedMessagesNext::Done,
            ]
        );
        assert!(
            !hook.call_tools_had_tool_call.load(Ordering::SeqCst),
            "on_tool_call must not run before the CallTools checkpoint"
        );
        assert!(
            hook.call_tools_had_assistant.load(Ordering::SeqCst),
            "CallTools checkpoint must already include the assistant tool-call message"
        );
        assert!(hook.saw_tool_call.load(Ordering::SeqCst));
        let call_tools = events
            .iter()
            .find(|(_, next)| *next == CommittedMessagesNext::CallTools);
        assert!(
            matches!(call_tools, Some((len, _)) if *len >= 2),
            "CallTools messages should include prompt + assistant, got {events:?}"
        );
        assert_eq!(
            events.last().map(|(_, next)| *next),
            Some(CommittedMessagesNext::Done)
        );
    }

    #[tokio::test]
    async fn on_messages_committed_failed_after_max_turns() {
        let hook = RecordingCommittedHook::new();
        let agent = AgentBuilder::new(tool_then_text_model())
            .tool(MockAddTool)
            .add_hook(hook.clone())
        ...
        let err = agent
            ...
            .max_turns(1)
            .await
            .expect_err("max_turns=1 should fail before the second model call");
        assert!(matches!(
            err,
            PromptError::MaxTurnsError { max_turns: 1, .. }
        ));

        let nexts: Vec<_> = hook.events().into_iter().map(|(_, next)| next).collect();
        assert_eq!(
            nexts,
            vec![
                CommittedMessagesNext::CallModel,
                CommittedMessagesNext::CallTools,
                CommittedMessagesNext::Failed,
            ]
        );
        assert!(
            !hook.call_tools_had_tool_call.load(Ordering::SeqCst),
            "CallTools checkpoint must precede on_tool_call even when the run later fails"
        );
        assert!(hook.call_tools_had_assistant.load(Ordering::SeqCst));
    }

    #[derive(Clone, Default)]
    struct StopOnCallTools {
        saw_tool_call: Arc<AtomicBool>,
    }

    impl AgentHook for StopOnCallTools {
        async fn on_messages_committed(
            &self,
            _ctx: &HookContext,
            event: CommittedMessages<'_>,
            ...
            if event.next == CommittedMessagesNext::CallTools {
                return CommittedMessagesAction::stop("persist failed");
            }
            CommittedMessagesAction::continue_run()
        }
    ...
    #[tokio::test]
    async fn persist_stop_at_call_tools_does_not_execute_tools() {
        let hook = StopOnCallTools::default();
        let agent = AgentBuilder::new(tool_then_text_model())
            .tool(MockAddTool)
            .add_hook(hook.clone())
            .build();
        ...
        let err = agent
            ...
            .max_turns(2)
            .await
            .expect_err("CallTools persist Stop should cancel the run");
        assert!(matches!(
            err,
            PromptError::PromptCancelled { ref reason, .. } if reason == "persist failed"
        ));
        assert!(
            !hook.saw_tool_call.load(Ordering::SeqCst),
            "tools must not run after a persist Stop at CallTools"
        );
    }
}
