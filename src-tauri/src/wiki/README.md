...
正文默认在 `~/.codeg/wiki/`（或 `$CODEG_HOME/wiki`），原件、日志与暂存在对应的 `wiki-state/`。不把桌面应用的 `CODEG_DATA_DIR` 当作 Wiki 默认目录；用户可单独指定正文目录，状态目录位置不因此改变。设置只读取 `wiki_settings`，目录标记使用 `.codeg-wiki.json`；不以目录名区分数据版本，模型绑定独立保存。
